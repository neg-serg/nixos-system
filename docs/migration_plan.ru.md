...
- [x] Служебные скрипты (например, `scripts/hm-hy3-system.apply.sh`) и Just‑таргеты теперь по
  умолчанию смотрят на `/etc/nixos/home`, чтобы не требовать отдельного клонa `~/.dotfiles`.
- [ ] Остаточные ссылки на `~/.dotfiles` и старые `home-manager switch` сценарии нужно постепенно
  удалить или пометить как требующие ручного режима (см. вывод `rg '.dotfiles'`).
