...
Purpose: monitor Nextcloud availability and latency via Blackbox, and PHP-FPM pool health via the Prometheus php-fpm exporter. This complements the existing Loki logs and system metrics.

## What’s included in this repo

- Blackbox HTTPS probe to `https://telfir/status.php` with permissive TLS (LAN/self-signed). Exposed as job `blackbox-https-insecure` pointing to the Blackbox exporter on localhost.
- Optional PHP-FPM exporter configured to scrape the Nextcloud pool through its unix socket with status path `/status`.
...
- Blackbox probe: open Prometheus Targets and check `blackbox-https-insecure` instance `https://telfir/status.php`.
- PHP-FPM exporter: `curl -s http://127.0.0.1:9253/metrics | head`.

## Grafana: useful queries

Use autocomplete for prefixes `probe_` and `php_fpm_` as metric names vary slightly per exporter version.
...
- High Nextcloud latency (example):
  - `probe_duration_seconds{job="blackbox-https-insecure", instance="https://telfir/status.php"} > 2` for 5m
