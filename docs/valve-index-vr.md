...
- Execute `vkcube` and `vkcube --display` to validate Vulkan acceleration on both native and X11
  fallback paths.
- For controller mappings, check `~/.local/share/Steam/config/steamvr.vrsettings` after pairing to
  ensure input profiles loaded.
