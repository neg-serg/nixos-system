...
- Перезагрузите систему один раз, чтобы параметры ядра, правила udev и пользовательские сервисы
  корректно подхватились.
...
- Запустите SteamVR; проверьте, что композитор стартует и показывает домашнее пространство без
  ошибок.
...
- Для раскладок контроллеров проверьте `~/.local/share/Steam/config/steamvr.vrsettings` после
  привязки, чтобы убедиться, что профили ввода подхватились.
