{pkgs, lib, config, ...}: let
  ...
  gamescopePinned = pkgs.writers.writePython3Bin "gamescope-pinned" {}
    ...
    cmd = [TASKSET, "-c", cpuset, GAMEMODERUN, GAMESCOPE] + shlex.split(flags) + ["--"] + sys.argv[1:]
  ...
  gamescopePerf = pkgs.writers.writePython3Bin "gamescope-perf" {}
    ''
    import os, json, math, shlex, subprocess, sys
    H = {
      "HYPRCTL": "hyprctl",
    ...
    def pick_monitor(mon_name, mons):
      if mons:
        ...
        return sorted(mons, key=lambda m: (m.get("refreshRate",0), (m.get("width",0) * m.get("height",0))), reverse=True)[0]
      return None
    ...
    if mon_env:
      try:
        subprocess.run([H["HYPRCTL"], "dispatch", "focusmonitor", mon_env], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
      except Exception:
        pass
    ...
    if rate:
      flags += ["-r", rate]
    flags += ["-w", game_w, "-h", game_h, "-W", out_w, "-H", out_h, "--fsr-sharpness", "3"]

    if len(sys.argv) == 1:
      try:
        cmd_str = subprocess.check_output([H["ZENITY"], "--entry", "--title=Gamescope Performance", "--text=Command to run:"], text=True).strip()
      except Exception:
        cmd_str = ""
      if not cmd_str:
        sys.exit(0)
    ...
    cmd = [H["TASKSET"], "-c", cpuset, H["GAMEMODERUN"], H["GAMESCOPE"]] + flags + ["--"] + args
    raise SystemExit(subprocess.call(cmd))
    '';
  ...
  gamescopeQuality = pkgs.writers.writePython3Bin "gamescope-quality" {}
    ...
    if rate:
      flags += ["-r", rate]
    flags += ["-W", out_w, "-H", out_h]

    if len(sys.argv) == 1:
      try:
        cmd_str = subprocess.check_output([H["ZENITY"], "--entry", "--title=Gamescope Quality", "--text=Command to run:"], text=True).strip()
      except Exception:
        cmd_str = ""
      if not cmd_str:
        sys.exit(0)
  ...
  gamescopeHDR = pkgs.writers.writePython3Bin "gamescope-hdr" {}
    ...
    if len(sys.argv) == 1:
      try:
        cmd_str = subprocess.check_output([H["ZENITY"], "--entry", "--title=Gamescope HDR", "--text=Command to run:"], text=True).strip()
      except Exception:
        cmd_str = ""
      if not cmd_str:
        sys.exit(0)
  ...
  gamescopeTargetFPS = pkgs.writers.writePython3Bin "gamescope-targetfps" {}
    ...
    if len(sys.argv) == 1:
      ...
      prompt += "):"
      try:
        cmd_str = subprocess.check_output([H["ZENITY"], "--entry", "--title=Gamescope Target FPS", f"--text={prompt}"], text=True).strip()
      except Exception:
        cmd_str = ""
      if not cmd_str:
        sys.exit(0)
