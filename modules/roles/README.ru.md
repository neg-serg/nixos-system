...
- Переопределяйте тумблеры сервисов через `profiles.services.<name>.enable` (алиас: `servicesProfiles.<name>.enable`).
...
- Типичные дальнейшие шаги по ролям:
  ...
  - Homelab: задать DNS‑переписывания в `servicesProfiles.adguardhome.rewrites`, определить устройства/папки Syncthing в `hosts/<host>/services.nix`.
  - Media: указать пути/порты для MPD/Navidrome; Jellyfin открывает нужные порты сам при включении.

- Документация: агрегированные опции доступны через flake‑артефакт `packages.${system}."options-md"`, когда собран.
